...
function refresh_sceneitem(find_newest)
    ...
    if find_newest then
        ...
        log("Finding sceneitem for 放大源 '" .. source_name .. "'")
    ...
    if is_non_display_capture then
        if not use_monitor_override then
            log("ERROR: Selected 放大源 is not a display capture source.\n" ..
...
function on_print_help()
    local help = "\n----------------------------------------------------\n" ..
        "OBS 鼠标跟随放大脚本帮助信息 v" .. VERSION .. "\n" ..
        "https://github.com/BlankSourceCode/obs-zoom-to-mouse\n" ..
        "----------------------------------------------------\n" ..
        "本脚本会将选中的显示器采集源放大，并聚焦到鼠标位置\n\n" ..
        "放大源：当前场景中用于放大的显示器采集源\n" ..
        "放大倍数：画面放大的倍率\n" ..
        "放大速度：放大/缩小动画的速度\n" ..
        "自动跟随鼠标：启用后，放大状态下会跟随鼠标\n" ..
        "允许边界外跟随：启用后，鼠标移出来源范围也继续跟随\n" ..
        "跟随速度：放大画面跟随鼠标移动的速度\n" ..
        "跟随边界：距离画面边缘多少百分比时重新启用鼠标跟随\n" ..
        "锁定灵敏度：跟随到多接近目标后锁定位置，直到鼠标进入跟随边界才继续跟随\n" ..
        "反向移动时自动锁定：鼠标反向移动时自动停止跟随\n" ..
        "显示所有来源：允许选择任意来源作为放大源；非显示器采集源必须手动设置来源位置\n" ..
        "手动设置来源位置：启用后用手动填写的 X/Y、宽/高、缩放 X/Y 覆盖自动计算结果\n" ..
        "X：来源最左侧像素的坐标\n" ..
        "Y：来源最上方像素的坐标\n" ..
        "宽度：来源宽度，单位像素\n" ..
        "高度：来源高度，单位像素\n" ..
        "缩放 X：来源不是 1:1 时应用到鼠标位置的横向缩放比例，克隆来源时常用\n" ..
        "缩放 Y：来源不是 1:1 时应用到鼠标位置的纵向缩放比例，克隆来源时常用\n" ..
        "显示器宽度：显示该来源的显示器宽度，单位像素\n" ..
        "显示器高度：显示该来源的显示器高度，单位像素\n"

    if socket_available then
        help = help ..
            "启用远程鼠标监听：开启 UDP 监听服务，用于接收远程客户端发送的鼠标位置，参考：https://github.com/BlankSourceCode/obs-zoom-to-mouse-remote\n" ..
            "端口：UDP 监听使用的端口号\n" ..
            "轮询间隔：更新鼠标位置的间隔，单位毫秒\n"
    end
    ...
    help = help ..
        "更多说明：在脚本日志中显示这段帮助文字\n" ..
        "启用调试日志：在脚本日志中显示更多调试信息\n\n"

    obs.script_log(obs.OBS_LOG_INFO, help)
end

function script_description()
    return "将选中的显示器采集源放大，并聚焦到鼠标位置"
end
...
function script_properties()
    ...
    local sources_list = obs.obs_properties_add_list(props, "source", "放大源", obs.OBS_COMBO_TYPE_LIST,
        obs.OBS_COMBO_FORMAT_STRING)

    populate_zoom_sources(sources_list)

    local refresh_sources = obs.obs_properties_add_button(props, "refresh", "刷新放大源列表",
        function()
            populate_zoom_sources(sources_list)
            monitor_info = get_monitor_info(source)
            return true
    ...
    obs.obs_property_set_long_description(refresh_sources,
        "点击后重新扫描可用来源，并更新“放大源”下拉列表")

    -- Add the rest of the settings UI
    local zoom = obs.obs_properties_add_float(props, "zoom_value", "放大倍数", 1, 5, 0.5)
    local zoom_speed = obs.obs_properties_add_float_slider(props, "zoom_speed", "放大速度", 0.01, 1, 0.01)
    local follow = obs.obs_properties_add_bool(props, "follow", "自动跟随鼠标 ")
    obs.obs_property_set_long_description(follow,
        "启用后，放大时会自动开始跟随鼠标，无需再按“跟随鼠标”热键")

    local follow_outside_bounds = obs.obs_properties_add_bool(props, "follow_outside_bounds", "允许边界外跟随 ")
    obs.obs_property_set_long_description(follow_outside_bounds,
        "启用后，即使鼠标移出放大源范围，也会继续跟随鼠标位置")
    ...
    obs.obs_property_set_long_description(follow_auto_lock,
        "启用后，鼠标移动到放大画面边缘会开始跟随，\n" ..
        "但鼠标往中心移动时会停止跟随，类似即时战略游戏里的镜头平移")

    local allow_all = obs.obs_properties_add_bool(props, "allow_all_sources", "允许任意来源作为放大源 ")
    obs.obs_property_set_long_description(allow_all, "启用后可以选择任意来源作为放大源\n" ..
        "如果选择的不是“显示器采集”来源，必须手动设置来源位置")
    ...
    local override = obs.obs_properties_add_group(props, "use_monitor_override", "手动设置来源位置 ",
        obs.OBS_GROUP_CHECKABLE, override_props)

    obs.obs_property_set_long_description(override_label,
        "启用后，将使用下面手动填写的位置和尺寸参数，而不是脚本自动计算的结果")
    obs.obs_property_set_long_description(override_sx, "通常填 1；如果来源被缩放过，再按实际比例填写")
    obs.obs_property_set_long_description(override_sy, "通常填 1；如果来源被缩放过，再按实际比例填写")
    obs.obs_property_set_long_description(override_dw, "显示器的横向分辨率")
    obs.obs_property_set_long_description(override_dh, "显示器的纵向分辨率")
    ...
    if socket_available then
        ...
        local r_poll = obs.obs_properties_add_int(socket_props, "socket_poll", "轮询间隔（毫秒） ", 0, 1000, 1)
        ...
        obs.obs_property_set_long_description(r_label,
            "启用后会开启 UDP 监听，用于接收远程客户端发送的鼠标位置")
        obs.obs_property_set_long_description(r_port,
            "修改端口后需要重启监听：取消勾选后再重新勾选“启用远程鼠标监听”")
        obs.obs_property_set_long_description(r_poll,
            "修改轮询间隔后需要重启监听：取消勾选后再重新勾选“启用远程鼠标监听”")
...
function script_load(settings)
    ...
    -- Add our hotkey
    hotkey_zoom_id = obs.obs_hotkey_register_frontend("toggle_zoom_hotkey", "切换鼠标位置放大",
        on_toggle_zoom)

    hotkey_follow_id = obs.obs_hotkey_register_frontend("toggle_follow_hotkey", "放大时切换鼠标跟随",
        on_toggle_follow)
...
function populate_zoom_sources(list)
    ...
    if sources ~= nil then
        local dc_info = get_dc_info()
        obs.obs_property_list_add_string(list, "<不选择>", "obs-zoom-to-mouse-none")
